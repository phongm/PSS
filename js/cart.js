...
//					}
				...
				$.each($(".cart_num_input"), function(index,element) {
					...
					if($(this).val() < 9){
						$(this).siblings(".cart_num_add").removeClass("disable");
					}else{
						$(this).siblings(".cart_num_add").addClass("disable").attr("disabled","disabled");
					}
					//商品数量的修改
					//商品增加
					$(".cart_num_add").eq(index).on("click",function(){
						if($(this).siblings($(".cart_num_input")).val() <= 8){
							//改变样式
							$(this).siblings("input").val(parseInt($(this).siblings("input").val())+1);
							//最大不超过9
							parseInt($(this).siblings("input").val()) > 9 && $(this).siblings("input").val(9);
							//改变cookie
